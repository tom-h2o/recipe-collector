-- Sync all AI prompt templates to their latest canonical version.
-- These values mirror api/_lib/prompts.ts exactly.
-- Each stored value is the instruction/rules section ONLY — runtime data
-- (recipe title, ingredients, instructions etc.) is appended by the
-- corresponding build*Prompt() helper in each API endpoint.
--
-- Changes vs previous migrations:
--   - gemini_prompt_nutrition: removed the duplicate JSON return-format block
--     that 0031 accidentally included (it is already appended at runtime by
--     buildNutritionPrompt() in api/nutrition.ts).
--   - gemini_prompt_translate: retains the unit-word translation rule from 0032
--     (e.g. "tablespoon" → "Esslöffel") with template-only format (no input block).
--   - All other prompts: re-stated for clarity / single source of truth.
...
- Return a JSON array of strings, nothing else. Example: ["Italian", "Quick (<30min)", "Vegetarian"]',
  ...
  -- NOTE: do NOT include the JSON return-format block here.
  -- buildNutritionPrompt() in api/nutrition.ts appends it at runtime.
...
- Calibration anchors: plain chicken breast 165g ≈ 280 kcal, 31g protein; 1 cup cooked pasta ≈ 220 kcal, 8g protein, 43g carbs; 1 tbsp olive oil ≈ 120 kcal, 14g fat.',
  ...
  -- NOTE: do NOT include the input recipe block or the return JSON structure here.
  -- buildTranslatePrompt() in api/translate.ts appends the target language,
  -- input recipe data, and return structure at runtime.
  gemini_prompt_translate = 'You are a professional culinary translator.
