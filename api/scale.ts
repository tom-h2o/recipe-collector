...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { ingredients, currentServings, targetServings } = scaleSchema.parse(req.body);
...
Preserve the "details" field from the original exactly. Only change "amount".`;
    ...
    const scaled = await generateJson<{ amount: string; name: string; details: string }[]>(
      client,
    ...
    return res.status(200).json({ ingredients: scaled });
  } catch (err: unknown) {
