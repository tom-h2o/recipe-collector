...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (body.action === 'send') {
      ...
      if (recipe.user_id && recipe.user_id !== senderUserId) return res.status(403).json({ error: 'You do not own this recipe.' });
      const { data: existing } = await supabase.from('recipe_shares').select('id').eq('recipe_id', recipeId).eq('recipient_email', recipientEmail.toLowerCase()).eq('status', 'pending').maybeSingle();
      if (existing) return res.status(409).json({ error: 'You already sent this recipe to that address — it is still pending.' });
      const { error: insertErr } = await supabase.from('recipe_shares').insert({ recipe_id: recipeId, recipe_title: recipe.title, recipe_description: recipe.description ?? null, recipe_image_url: recipe.image_url ?? null, sender_id: senderUserId, sender_email: senderEmail.toLowerCase(), recipient_email: recipientEmail.toLowerCase() });
      if (insertErr) throw insertErr;
      await supabase.from('contacts').upsert({ user_id: senderUserId, contact_email: recipientEmail.toLowerCase() }, { onConflict: 'user_id,contact_email' });
      return res.status(200).json({ ok: true });
    }

    if (body.action === 'accept') {
      const { shareId, recipientUserId, recipientEmail } = body;
      const { data: share, error: shareErr } = await supabase.from('recipe_shares').select('*').eq('id', shareId).eq('recipient_email', recipientEmail.toLowerCase()).eq('status', 'pending').single();
      ...
      await supabase.from('recipe_shares').update({ status: 'accepted' }).eq('id', shareId);
      await supabase.from('contacts').upsert({ user_id: recipientUserId, contact_email: share.sender_email }, { onConflict: 'user_id,contact_email' });
      return res.status(200).json({ ok: true, newRecipeId: newRecipe.id });
    }

    if (body.action === 'reject') {
      const { shareId, recipientEmail } = body;
      const { error: rejectErr } = await supabase.from('recipe_shares').update({ status: 'rejected' }).eq('id', shareId).eq('recipient_email', recipientEmail.toLowerCase()).eq('status', 'pending');
